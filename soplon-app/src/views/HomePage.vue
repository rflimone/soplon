<template>
  <v-ons-page>
    ...
    <div v-if="categories">
      ...
      <v-ons-col>
        <v-ons-card v-if="categories.length > getLastIndex(rowIndex)"  @click="goTo(categories[getLastIndex(rowIndex)].glosaCategoria)">
...
(function () {
  ...
  this.computed = {
    ...
    rows: function () {
      ...
      return parseInt(this._categories.length / 2) + this._categories.length % 2
    },
    ...mapGetters({
      token: 'auth/token'
    })
